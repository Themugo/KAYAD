...
describe("🚗 Car Listings", () => {
  ...
  test("GET /api/cars — respects ?limit cap (max 100)", async () => {
    const res = await request(app).get("/api/cars?limit=9999").expect(200);
  ...
  test("GET /api/cars — accepts valid filters", async () => {
    await request(app).get("/api/cars?brand=Toyota&fuel=Petrol&transmission=Automatic").expect(200);
  });

  test("GET /api/cars/:id — returns 404 for bad ID", async () => {
    await request(app).get("/api/cars/000000000000000000000000").expect(404);
  });

  test("POST /api/cars — requires auth", async () => {
    await request(app).post("/api/cars").send({ title: "Test Car", price: 500000 }).expect(401);
  });
});
