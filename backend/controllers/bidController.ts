...
import { emitListingUpdate } from "../socket/socket.ts";
import { sendSMS } from "../utils/sms.ts";
import { logActionFromReq } from "../utils/securityLogger.ts";
import { applySnipingProtection } from "../utils/snipeGuard.ts";
import { getIO } from "../utils/io.ts";
import { logInfo, logWarn, logError } from "../utils/logger.ts";
import { findOrCreateLeadFromAuction, addLeadActivity, updateLeadStage } from "../services/leadService.ts";
import { logAuctionBidPlaced } from "../services/auditService.ts";

// Email service — top-level import, no-ops if unavailable
let bidEmailService = {};
try {
  bidEmailService = await import("../services/email.service.ts");
} catch (e) {
...
export const confirmBidPayment = async (req, res) => {
  try {
    ...
    try {
      const { generateReceipt } = await import("../services/pdfService.ts");
      generateReceipt({
