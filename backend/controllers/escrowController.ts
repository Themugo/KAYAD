...
import { isValidId } from "../utils/validateId.ts";
import { findOrCreateLeadFromEscrow, updateLeadStage } from "../services/leadService.ts";
import { logEscrowReleased, logEscrowRefunded } from "../services/auditService.ts";
...
export const notifyEscrowReleased = async (escrow) => {
  try {
    const { sendEscrowReleasedEmail } = await import("../services/email.service.ts");
    const EscrowModel = (await import("../models/Escrow.ts")).default;
    const UserModel = (await import("../models/User.ts")).default;
...
export const notifyEscrowRefunded = async (escrow) => {
  try {
    const { sendEscrowRefundedEmail } = await import("../services/email.service.ts");
    const EscrowModel = (await import("../models/Escrow.ts")).default;
    const UserModel = (await import("../models/User.ts")).default;
    const populated = await EscrowModel.findById(escrow._id || escrow)
