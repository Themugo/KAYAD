...
let emailService = {};
try {
  emailService = await import("../services/email.service.ts");
} catch (e) {
  ...
  try {
    ...
    if (referredBy) {
      ...
      try {
        ...
        await (
          await import("../models/Referral.ts")
        ).default.create({
          referrer: referredBy,
          referee: user._id,
          status: "credited",
        ...
        const { sendNotification } = await import("../services/notification.service.ts");
        sendNotification({
          userId: referredBy,
