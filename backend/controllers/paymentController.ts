...
import { initiatePayment as initiate } from "../services/paymentService.ts";
import { handleMpesaCallback } from "../services/paymentCallback.service.ts";
  ...
  try {
    ...
    if (normalizedType === "escrow" && result.payment?._id) {
      ...
      if (car && car.escrowEnabled !== false) {
        const Escrow = (await import("../models/Escrow.ts")).default;
        ...
        try {
          const { findOrCreateLeadFromEscrow, updateLeadStage } = await import("../services/leadService.ts");
