...
export const webhookFundsReceived = async (req, res) => {
  try {
    ...
    const result = await withTransaction(async (session) => {
      ...
      if (!vault) {
        ...
        if (existing && existing.status !== "awaiting_payment") {
          return { success: true, message: "Already funded", vault: existing };
        }
      ...
      const car = vault.car;
      if (car) {
        await Car.findByIdAndUpdate(car._id, { status: "pending" }, { session });
      }
    ...
    if (!result.success && result.message) {
      return res.status(result.message === "Vault not found" ? 404 : 200).json(result);
    }
    ...
    logSecurityAction({
      ...
      resourceId: id,
      details: { carId: vault.car?._id || vault.car, amount: vault.amount, bankRef },
      severity: "info",
    });

    if (vault.car?._id) {
      emitListingUpdate(vault.car._id.toString(), { status: "pending", escrowStatus: "escrow_locked" });
    }

    await sendNotification({
      userId: vault.seller._id,
      title: "💰 Funds Secured in Escrow Vault",
      message: `KES ${Number(vault.amount).toLocaleString("en-KE")} is secured in the KAYAD escrow vault for your listing. You can now safely release the vehicle for physical inspection and NTSA TIMS verification.`,
      type: "escrow",
...
export const adminConfirmFunding = async (req, res) => {
  try {
    ...
    await withTransaction(async (session) => {
      const vault = await EscrowVault.findById(id).session(session);
      ...
      if (vault.car) {
        await Car.findByIdAndUpdate(vault.car, { status: "pending" }, { session });
      }
    ...
    await sendNotification({
      userId: vault.seller._id,
      title: "💰 Funds Secured in Escrow Vault",
      message: `KES ${Number(vault.amount).toLocaleString("en-KE")} confirmed in escrow for your listing. Vehicle can now be released for inspection.`,
      type: "escrow",
    });
...
export const requestReleaseOtp = async (req, res) => {
  try {
    ...
    if (phone) {
      logInfo(`Release OTP sent to ${phone.slice(0, -4)}****`);
    }
...
export const releaseWithOtp = async (req, res) => {
  try {
    ...
    await withTransaction(async (session) => {
      ...
      if (car) {
        await Car.findByIdAndUpdate(
          car._id,
          { sold: true, status: "sold", paymentStatus: "released", isPaid: true },
          { session },
        );
      }
    });
    ...
    if (vault.car?._id) {
      emitListingUpdate(vault.car._id.toString(), { status: "sold", sold: true, paymentStatus: "released" });
    }
