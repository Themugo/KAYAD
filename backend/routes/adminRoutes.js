...
router.get(
  ...
  adminOrSuper,
  asyncHandler(async (req, res) => {
    const [demoUsers, demoUsersDeleted, demoCars, demoBids, demoPayments, demoEscrows, config] = await Promise.all([
      ...
      PlatformConfig.findOne().select("demoMode").lean(),
    ]);
...
// =============================
// 🔄 TOGGLE DEMO MODE (admin/superadmin)
// =============================
router.post(
  "/demo/toggle",
  adminOrSuper,
  asyncHandler(async (req, res) => {
    ...
    await config.save();
    await AuditLog.create({
      action: `Demo mode ${config.demoMode ? "enabled" : "disabled"}`,
      admin: req.user.name || req.user.email,
      adminId: req.user.id,
    });
    res.json({ success: true, demoMode: config.demoMode });
  }),
);

// =============================
// 🌱 RESEED DEMO DATA (admin/superadmin)
// =============================
router.post(
  "/demo/reseed",
  adminOrSuper,
  asyncHandler(async (req, res) => {
    ...
    const result = await reseed();
    await AuditLog.create({
      action: "Demo data reseeded",
      admin: req.user.name || req.user.email,
      adminId: req.user.id,
      details: { cars: result.cars },
    });
    res.json({ success: true, message: "Demo data reseeded", result });
  }),
);
