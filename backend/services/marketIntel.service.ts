...
export async function getMarketPulse(carId, car) {
  ...
  const fairPriceBase = Math.round(avgPrice * brandPremium * depFactor - mileageImpact);
  ...
  const recommendations = await Promise.all(
    cars.slice(0, 5).map(async (c) => {
      ...
      return {
        ...
        daysOnMarket: c.createdAt ? Math.round((Date.now() - new Date(c.createdAt).getTime()) / 86400000) : 0,
      };
    }),
  );
