import { sendSMS as _sendSMS } from "../utils/sms.ts";
import { logInfo, logError } from "../utils/logger.ts";
import { addSMSJob } from "../queues/smsQueue.ts";

const QUEUE_MODE = process.env.QUEUE_MODE === "true";

export { sendSMS } from "../utils/sms.ts";
...
export const sendWelcomeSMS = (phone, name) =>
  sms(
    phone,
    `Welcome to Kayad, ${name}! Start exploring premium cars, auctions, and more. Download the app: kayad.space`,
  );

export const sendBidPlacedSMS = (phone, carTitle, amount) =>
  sms(phone, `Bid confirmed on ${carTitle || "vehicle"} — ${formatKES(amount)}. Track it live on Kayad.`, {
    carTitle,
    amount,
  });

export const sendOutbidSMS = (phone, carTitle, amount) =>
  sms(phone, `You've been outbid on ${carTitle || "vehicle"} — ${formatKES(amount)}. Bid higher now on Kayad.`, {
    carTitle,
    amount,
  });
...
export const sendEscrowReleasedSMS = (phone, amount, carTitle) =>
  sms(
    phone,
    `Escrow released — ${formatKES(amount)} for ${carTitle || "vehicle"} has been paid to the seller. Kayad.`,
    { amount, carTitle },
  );
...
export const sendEscrowRefundedSMS = (phone, amount, carTitle) =>
  sms(
    phone,
    `Escrow refunded — ${formatKES(amount)} for ${carTitle || "vehicle"} has been returned to your M-Pesa. Kayad.`,
    { amount, carTitle },
  );
...
export const sendOTPSMS = (phone, otp) => sms(phone, `Your KAYAD verification code is: ${otp}`, { otp });
