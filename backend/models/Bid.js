...
bidSchema.pre("aggregate", function () {
...
// ⚡ STATIC: SET WINNER
// =============================
bidSchema.statics.markWinner = async function (bidId, session) {
  const opts = session ? { session } : {};
  const bid = await this.findById(bidId).session(session || null);
  if (!bid) return null;

  await this.updateMany({ carId: bid.carId }, { isWinningBid: false }).session(session || null);

  return this.findByIdAndUpdate(bidId, { isWinningBid: true }, { new: true, ...opts });
};
