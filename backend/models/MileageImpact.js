...
mileageImpactSchema.methods.getPricePerMile = function (mileage) {
  const range = this.mileageRanges.find(
    r => mileage >= r.minMileage && mileage <= r.maxMileage
  );
...
// Add mileage range
mileageImpactSchema.methods.addMileageRange = function (minMileage, maxMileage, averagePrice, pricePerMile, depreciationFactor, sampleSize) {
  const existingRange = this.mileageRanges.find(
    r => r.minMileage === minMileage && r.maxMileage === maxMileage
  );
...
mileageImpactSchema.statics.getByBodyType = async function (bodyType) {
  return this.find({ "vehicle.bodyType": bodyType })
    .lean();
};
...
mileageImpactSchema.statics.calculateFromValuations = async function (filters = {}) {
  ...
  const rangeData = mileageRanges.map(range => {
  ...
  const pricePer1000Miles = rangeData.length > 0 
    ? rangeData.reduce((sum, r) => sum + r.pricePerMile, 0) / rangeData.length * 1000
    : 0;
