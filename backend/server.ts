// backend/server.ts — PRODUCTION HARDENED v3.0
// =============================================
...
import { globalLimiter, authLimiter, adminLimiter, socketRateLimit } from "./middleware/rateLimiter.ts";

// ─── Security Middleware ───────────────────────────────────────
import { mongoSanitize, xssProtection, paginationCap, extraHeaders, bodyGuard } from "./middleware/security.ts";
import { mpesaIpWhitelist, validateMpesaCallback } from "./middleware/mpesaSecurity.ts";
import { checkSystemStatus } from "./middleware/systemCheck.ts";
import { csrfProtection } from "./middleware/csrf.ts";
import { idempotencyCheck } from "./middleware/idempotency.ts";
import { protect, adminOnly } from "./middleware/auth.ts";
import responseWrapper from "./middleware/responseWrapper.ts";
...
import dealerRoutes from "./routes/dealerRoutes.ts";
import adminRoutes from "./routes/adminRoutes.ts";
import paymentRoutes from "./routes/paymentRoutes.ts";
import escrowRoutes from "./routes/escrowRoutes.ts";
...
import auctionAdminRoutes from "./routes/auctionAdminRoutes.ts";
import adRoutes from "./routes/adRoutes.ts";
import userRoutes from "./routes/userRoutes.ts";
import savedSearchRoutes from "./routes/savedSearchRoutes.ts";
import ntsaVerificationRoutes from "./routes/ntsaVerificationRoutes.ts";
import inspectionRoutes from "./routes/inspectionRoutes.ts";
import escrowVaultRoutes from "./routes/escrowVaultRoutes.ts";
import referralRoutes from "./routes/referralRoutes.ts";
import securityLogRoutes from "./routes/securityLogRoutes.ts";
import smsBiddingRoutes from "./routes/smsBiddingRoutes.ts";
import inspectorApplicationRoutes from "./routes/inspectorApplicationRoutes.ts";
import contactRoutes from "./routes/contactRoutes.ts";
import conversionFunnelRoutes from "./routes/conversionFunnelRoutes.ts";
import disputeRoutes from "./routes/disputeRoutes.ts";
import fraudRoutes from "./routes/fraudRoutes.ts";
import operationsRoutes from "./routes/operationsRoutes.ts";
import supportRoutes from "./routes/supportRoutes.ts";
import eventRoutes from "./routes/eventRoutes.ts";
import executiveAnalyticsRoutes from "./routes/executiveAnalyticsRoutes.ts";
import subscriptionRoutes from "./routes/subscriptionRoutes.ts";
import listingAssistantRoutes from "./routes/listingAssistantRoutes.ts";
import recommendationRoutes from "./routes/recommendationRoutes.ts";
import marketRoutes from "./routes/marketRoutes.ts";
import verificationRoutes from "./routes/verificationRoutes.ts";
import duplicateRoutes from "./routes/duplicateRoutes.ts";
import auditRoutes from "./routes/auditRoutes.ts";
import dealerHealthScoreRoutes from "./routes/dealerHealthScoreRoutes.ts";
import leadRoutes from "./routes/leadRoutes.ts";
import vehicleAnalyticsRoutes from "./routes/vehicleAnalyticsRoutes.ts";
import marketplaceHealthRoutes from "./routes/marketplaceHealthRoutes.ts";
import featureFlagRoutes from "./routes/featureFlagRoutes.ts";
import searchAnalyticsRoutes from "./routes/searchAnalyticsRoutes.ts";
import listingQualityRoutes from "./routes/listingQualityRoutes.ts";
import notificationAnalyticsRoutes from "./routes/notificationAnalyticsRoutes.ts";
...
import reconciliationRoutes from "./routes/reconciliationRoutes.ts";
import queueRoutes from "./routes/queueRoutes.ts";
import operationsDashboardRoutes from "./routes/operationsDashboardRoutes.ts";
...
import { logInfo, logWarn, logError, logDebug } from "./utils/logger.ts";
import { startAuctionEngine } from "./realtime/auctionEngine.ts";
import { startAuctionTimer } from "./utils/auctionTimer.ts";
import { startEscrowCron } from "./services/escrowCron.ts";
import { startAuctionReminderCron } from "./services/auctionReminderCron.ts";
import { startSavedSearchCron } from "./services/savedSearchCron.ts";
import { startPriceAlertCron } from "./services/priceAlertCron.ts";
import { startScheduler as startHealthScoreScheduler } from "./services/dealerHealthScoreScheduler.ts";
import { startScheduler as startMarketTrendScheduler } from "./services/marketTrendScheduler.ts";
import { startScheduler as startMarketplaceHealthScheduler } from "./services/marketplaceHealthScheduler.ts";
import { initPostHog } from "./utils/posthog.ts";
import { initCache } from "./utils/cache.ts";
import { registerHealthRoutes } from "./utils/healthCheck.ts";
import { getEnv, validateEnv } from "./utils/env.ts";
import { isRedisConnected } from "./utils/cache.ts";
import redisClient from "./config/redis.ts";
import { initSentry } from "./config/sentry.ts";
import { triggerAlert, ALERT_LEVELS } from "./config/alerting.ts";
import { recordHttpRequest, recordError } from "./config/metrics.ts";
const getRedisClient = () => redisClient;
import { setIO } from "./utils/io.ts";
...
import { mediumTimeout, fastTimeout, externalTimeout, uploadTimeout } from "./middleware/timeout.ts";
...
// ─── ENV VALIDATION ───────────────────────────────────────────
// (validateEnv is imported from ./utils/env.ts)
  ...
  try {
    ...
    try {
      ...
      if (existing === 0) {
        ...
        const { reseed } = await import("./seed.ts");
    ...
    if (isRedisConnected()) {
      ...
      setInterval(async () => {
        try {
          ...
          const Car = (await import("./models/Car.ts")).default;
...
const shutdown = async (signal) => {
  ...
  server.close(async () => {
    ...
    try {
      ...
      // Close queue workers
      const { closeWorkers } = await import("./config/queue.ts");
      await closeWorkers();
      logInfo("Queue workers closed");

      // Close dead letter queues
      const { closeDeadLetterQueues } = await import("./config/queue.ts");
      await closeDeadLetterQueues();
      logInfo("Dead letter queues closed");

      // Close queues
      const { closeQueues } = await import("./config/queue.ts");
      await closeQueues();
      logInfo("Queues closed");

      // Close Redis connection
      const { closeConnection } = await import("./config/queue.ts");
