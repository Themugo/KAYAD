...
export const down = async () => {
  try {
    ...
    await mongoose.connection.db
      .collection("proxybids")
      .dropIndex({ auctionId: 1, maxBid: -1 })
      .catch(() => {});
    console.log("✓ Dropped ProxyBid index: { auctionId: 1, maxBid: -1 }");

    await mongoose.connection.db
      .collection("proxybids")
      .dropIndex({ userId: 1, auctionId: 1 })
      .catch(() => {});
    console.log("✓ Dropped ProxyBid index: { userId: 1, auctionId: 1 }");

    await mongoose.connection.db
      .collection("proxybids")
      .dropIndex({ auctionId: 1, userId: 1, maxBid: -1 })
      .catch(() => {});
    ...
    await mongoose.connection.db
      ...
      .dropIndex({ user: 1, read: 1, createdAt: -1 })
      .catch(() => {});
