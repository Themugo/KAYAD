...
export const down = async () => {
  try {
    ...
    await mongoose.connection.db
      .collection("auctions")
      .dropIndex({ status: 1, endTime: 1, startTime: -1 })
      .catch(() => {});
    console.log("✓ Dropped Auction index: { status: 1, endTime: 1, startTime: -1 }");

    await mongoose.connection.db
      .collection("auctions")
      .dropIndex({ status: 1, paymentDeadline: 1 })
      .catch(() => {});
    ...
    await mongoose.connection.db
      .collection("chats")
      .dropIndex({ participants: 1, updatedAt: -1 })
      .catch(() => {});
    console.log("✓ Dropped Chat index: { participants: 1, updatedAt: -1 }");

    await mongoose.connection.db
      .collection("chats")
      .dropIndex({ car: 1, participants: 1 })
      .catch(() => {});
    ...
    await mongoose.connection.db
      .collection("leads")
      .dropIndex({ dealer: 1, isArchived: 1, stage: 1 })
      .catch(() => {});
