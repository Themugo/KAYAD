...
export const up = async () => {
  try {
    ...
    await mongoose.connection.db.collection('notifications_archived').createIndex(
      { createdAt: 1 }, 
      { expireAfterSeconds: 365 * 24 * 60 * 60, name: 'notifications_archived_ttl' }
    );
    ...
    for (const index of carIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('cars_archived').createIndex(
            index.key,
            { name: index.name }
          );
    ...
    for (const index of bidIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('bids_archived').createIndex(
            index.key,
            { name: index.name }
          );
    ...
    for (const index of paymentIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('payments_archived').createIndex(
            index.key,
            { name: index.name }
          );
    ...
    for (const index of escrowIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('escrows_archived').createIndex(
            index.key,
            { name: index.name }
          );
    ...
    for (const index of chatIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('chats_archived').createIndex(
            index.key,
            { name: index.name }
          );
    ...
    for (const index of leadIndexes) {
      if (index.name !== '_id_') {
        try {
          await mongoose.connection.db.collection('leads_archived').createIndex(
            index.key,
            { name: index.name }
          );
