...
import { detectFraud } from "../services/fraud.service.ts";
import { emitBidUpdate, emitAuctionEnd } from "../socket/socket.ts";
import { SNIPE_WINDOW_MS, EXTENSION_MS, applyRedisSnipingProtection } from "../utils/snipeGuard.ts";

import { syncBidToMongo, syncAuctionEnd } from "../services/auctionSync.service.ts";
...
export const endAuction = async (roomId) => {
  try {
    ...
    try {
      const { sendAuctionWonEmail } = await import("../services/email.service.ts");
      const User = (await import("../models/User.ts")).default;
      const CarModel = (await import("../models/Car.ts")).default;
