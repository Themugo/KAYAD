-- Dispute opening also links the authoritative inspection (when present) and
-- the canonical buyer/seller chat. No dispute table is created.
...
BEGIN
 ...
 IF v.car IS NOT NULL THEN
   SELECT id INTO v_chat FROM chats WHERE car=v.car AND participants=v_participants LIMIT 1;
   IF v_chat IS NULL THEN INSERT INTO chats(participants,car,messages,"createdAt","updatedAt") VALUES(v_participants,v.car,'[]'::jsonb,v_now,v_now) RETURNING id INTO v_chat; END IF;
   UPDATE escrows SET disputeChatId=v_chat WHERE id=p_escrow_id;
 END IF;
