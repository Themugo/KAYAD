...
export function filterDemoCars(filters = {}) {
  // Normalize param names: the backend uses `keyword`, frontend filter uses `search`
  if (filters.keyword && !filters.search) filters.search = filters.keyword;
  let r = [..._cars];
  if (filters.search) {
  ...
  if (filters.dealerType === 'dealer') r = r.filter(c => c.dealer?.role?.includes?.('dealer') || (!c.dealer?.role?.includes?.('individual_seller') && !c.dealer?.role?.includes?.('user')));
  if (filters.dealerType === 'private') r = r.filter(c => !c.dealer || c.dealer?.role === 'individual_seller' || c.dealer?.role === 'user');
