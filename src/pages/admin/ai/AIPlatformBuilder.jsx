...
export default function AIPlatformBuilder() {
  ...
  const loadData = async () => {
    try {
      ...
      setDashboard({
        ...
        totalPrompts: 24,
        knowledgeBaseSize: 89,
      });
      setHistory([
        { id: '1', command: 'Create a Toyota promotion page', action: 'create_page', status: 'executed', createdAt: new Date().toISOString() },
        { id: '2', command: 'Make the homepage more premium', action: 'suggest_design', status: 'executed', createdAt: new Date(Date.now() - 3600000).toISOString() },
        { id: '3', command: 'Launch Black Friday campaign', action: 'create_campaign', status: 'executed', createdAt: new Date(Date.now() - 7200000).toISOString() },
        { id: '4', command: 'Change primary color to blue', action: 'update_theme', status: 'pending', createdAt: new Date(Date.now() - 10800000).toISOString() },
      ]);
      setSuggestions([
        { id: '1', type: 'optimization', title: 'Optimize Homepage Hero', description: 'Video backgrounds show 34% higher engagement', command: 'Add video hero to homepage', impact: 'high', confidence: 87 },
        { id: '2', type: 'campaign', title: 'Launch Summer Promotion', description: 'Summer campaigns increase dealer inquiries by 25%', command: 'Create summer promotion', impact: 'medium', confidence: 82 },
        { id: '3', type: 'content', title: 'Add FAQ Section', description: 'Pages with FAQs have 40% lower bounce rates', command: 'Add FAQ to dealer page', impact: 'medium', confidence: 91 },
      ]);
      setHealth({
        overall: 'healthy',
        score: 95,
        categories: {
          pages: { status: 'healthy', score: 98 },
          images: { status: 'warning', score: 85 },
          seo: { status: 'healthy', score: 92 },
  ...
  const handleSend = async () => {
    ...
    try {
      const { data } = await aiApi.processAICommand({ command: input });
      
      const response = {
        role: 'assistant',
        content: data.preview?.changes?.[0]?.description || 'I\'ve processed your request. Here are the changes I\'ll make:',
      ...
      if (data.requiresApproval && data.commandId) {
        setPendingCommands(prev => [...prev, data]);
      }
    } catch (error) {
      const errorMessage = {
        role: 'assistant',
