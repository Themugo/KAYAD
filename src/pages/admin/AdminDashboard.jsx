        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
            {quickStats.map(qs => {
              const inner = (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontStyle: 'italic', fontSize: '1.5rem', color: '#fff', lineHeight: 1, marginBottom: 2 }}>
                        {formatValue(qs.value, qs.kes)}
                      </div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>{qs.label}</div>
                    </div>
                    <span style={{ fontSize: 24, lineHeight: 1, flexShrink: 0 }}>{qs.emoji}</span>
                  </div>
                  {qs.to && (
              ...
              return qs.to ? <Link key={qs.label} to={qs.to} style={{ textDecoration: 'none' }}>{inner}</Link> : inner;
            })}
          </div>
