...
const ORG = [
  { role: 'superadmin', title: 'Super Admin', icon: '👑', color: 'var(--gold)', bg: 'rgba(37, 99, 235,0.1)', desc: 'Full platform control. Manages all staff, config, and financials.', level: 0, canCreate: false },
        ...
        return (
          <div key={level} style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ height: 1, flex: 1, background: 'rgba(15, 23, 42, 0.05)' }} />
              <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em', color: 'rgba(15, 23, 42, 0.25)', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(levelRoles.length, 4)}, 1fr)`, gap: 14 }}>
              {levelRoles.map(r => {
                ...
                return (
                  <div key={r.role} style={{ background: '#FFFFFF', border: `1px solid ${r.color}20`, borderRadius: 14, padding: '20px', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: 'rgba(15, 23, 42, 0.35)', marginTop: 3, lineHeight: 1.5 }}>{r.desc}</div>
                      </div>
                    </div>
                    {members.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {members.map(m => (
                          <div key={m._id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: 'rgba(15, 23, 42, 0.04)', borderRadius: 8 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 600, color: '#0F172A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.name}</div>
                              <div style={{ fontSize: 10, color: 'rgba(15, 23, 42, 0.3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.email}</div>
                            </div>
                      ...
                      <div style={{ fontSize: 11, color: 'rgba(15, 23, 42, 0.2)', fontStyle: 'italic', textAlign: 'center', padding: '8px 0' }}>No staff assigned</div>
                    )}
