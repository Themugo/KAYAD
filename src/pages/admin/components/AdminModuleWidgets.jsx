...
export function StatBox({ icon: Icon, label, value, sub, color = 'var(--gold)', to }) {
  const inner = (
    <div role="presentation" style={{
      ...
      onMouseLeave={e => { if (to) { e.currentTarget.style.borderColor = 'rgba(15, 23, 42, 0.07)'; e.currentTarget.style.transform = 'none'; }}}
    >
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontStyle: 'italic', fontSize: '2rem', color: '#0F172A', lineHeight: 1, marginBottom: 4 }}>{value ?? '—'}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(15, 23, 42, 0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</div>
      {sub && <div style={{ fontSize: 11, color: 'rgba(15, 23, 42, 0.22)', marginTop: 4 }}>{sub}</div>}
    </div>
  );
...
export function NavTile({ to, icon: Icon, label, desc, danger }) {
  ...
  return (
    <Link to={to} style={{ textDecoration: 'none' }}>
      <div role="presentation" style={{
        background: '#FFFFFF',
        border: `1px solid ${danger ? 'rgba(239,68,68,0.12)' : 'rgba(15, 23, 42, 0.07)'}`,
        ...
        onMouseLeave={e => { e.currentTarget.style.borderColor = danger ? 'rgba(239,68,68,0.12)' : 'rgba(15, 23, 42, 0.07)'; e.currentTarget.style.background = '#FFFFFF'; }}
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(15, 23, 42, 0.3)' }}>{desc}</div>
        </div>
        <ChevronRight size={14} style={{ color: 'rgba(15, 23, 42, 0.2)', flexShrink: 0 }} />
      </div>
    </Link>
  );
}
