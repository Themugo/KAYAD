...
export default function AdminStaffList({ staff, loading, isSuperAdmin, onEdit, onToggleBan, onDelete, onShowAdd }) {
  ...
  if (staff.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '60px', background: '#FFFFFF', borderRadius: 16, border: '1px solid rgba(15, 23, 42, 0.06)' }}>
        <Users size={40} style={{ color: 'rgba(15, 23, 42, 0.15)', marginBottom: 14 }} />
        <div style={{ color: 'rgba(15, 23, 42, 0.3)', fontSize: 14 }}>No staff accounts yet</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {staff.map(m => {
        const orgRole = ORG.find(r => r.role === m.role) || { color: 'rgba(15, 23, 42, 0.4)', bg: 'rgba(15, 23, 42, 0.05)', title: m.role, icon: '👤' };
        return (
          <div key={m._id} style={{ background: '#FFFFFF', border: '1px solid rgba(15, 23, 42, 0.07)', borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: 'rgba(15, 23, 42, 0.4)' }}>{m.email}</div>
            </div>
            ...
            <div style={{ fontSize: 11, color: 'rgba(15, 23, 42, 0.25)' }}>
              {m.lastLogin ? new Date(m.lastLogin).toLocaleDateString('en-KE') : 'Never logged in'}
            </div>
            {isSuperAdmin && m.role !== 'superadmin' && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => onEdit(m)} style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(15, 23, 42, 0.05)', border: '1px solid rgba(15, 23, 42, 0.08)', color: 'rgba(15, 23, 42, 0.5)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Edit3 size={13} />
                </button>
