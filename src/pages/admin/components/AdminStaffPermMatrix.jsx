      ...
      <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, background: '#0C0C0C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
        ...
        <tbody>
          {ORG_DATA.map((r, i) => {
            ...
            return (
              <tr key={r.role}>
                <td style={{ padding: '12px 18px', borderBottom: i < ORG_DATA.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                ...
                {['users', 'cars', 'payments', 'escrow', 'config', 'staff', 'ads', 'auctions'].map(k => (
                  <td key={k} style={{ padding: '12px', textAlign: 'center', borderBottom: i < ORG_DATA.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
