      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', gap: 24 }}>
        ...
        <div style={{ flex: '0 0 400px', minWidth: 0 }}>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ color: 'rgba(15, 23, 42, 0.35)', fontSize: 13, marginTop: 6 }}>Read-only conversation view for dispute resolution</p>
          </div>

          <input placeholder="Search by participant name/email…"
            value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            style={{ width: '100%', background: 'rgba(15, 23, 42, 0.04)', border: '1px solid rgba(15, 23, 42, 0.1)', borderRadius: 8, padding: '10px 14px', color: '#0F172A', fontSize: 13, outline: 'none', marginBottom: 16 }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {chats.map(c => (
                <div key={c._id} onClick={() => loadMessages(c)}
                  ...
                  style={{
                    background: selectedChat?._id === c._id ? 'rgba(37, 99, 235,0.06)' : '#FFFFFF',
                    border: `1px solid ${selectedChat?._id === c._id ? 'rgba(37, 99, 235,0.2)' : 'rgba(15, 23, 42, 0.07)'}`,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {!selectedChat ? (
            ...
            <div>
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(15, 23, 42, 0.07)', borderRadius: 14, padding: '16px 20px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {messages.map((msg, idx) => (
                    <div key={msg._id || idx} style={{
                      background: '#FFFFFF', border: '1px solid rgba(15, 23, 42, 0.06)', borderRadius: 12, padding: '14px 18px',
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--gold)' }}>{msg.sender?.name || 'Unknown'}</div>
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          <span style={{ fontSize: 10, color: 'rgba(15, 23, 42, 0.25)' }}>{new Date(msg.createdAt || Date.now()).toLocaleString()}</span>
                      ...
                      <div style={{ fontSize: 13, color: msg.text === '[deleted by admin]' ? 'rgba(15, 23, 42, 0.25)' : 'rgba(15, 23, 42, 0.7)', fontStyle: msg.text === '[deleted by admin]' ? 'italic' : 'normal' }}>
                        {msg.text}
                      </div>
                      {msg.attachments?.length > 0 && (
                        <div style={{ marginTop: 8, fontSize: 11, color: 'rgba(15, 23, 42, 0.3)' }}>{msg.attachments.length} attachment(s)</div>
                      )}
                    </div>
                  ))}
                </div>
