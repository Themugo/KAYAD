      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        {isMobile && mobileFilterOpen && (
          <>
            ...
            <div style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: '85%', maxWidth: 320, background: '#070707', zIndex: 100, overflowY: 'auto', boxShadow: '4px 0 40px rgba(0,0,0,0.6)', animation: 'slideInLeft 0.25s ease' }}>
        ...
        <main style={{ flex: 1, minWidth: 0, background: '#050505' }}>
          ...
          <section style={{ padding: '0 0 48px' }}>
            <div className="container">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', marginBottom: 16, paddingTop: 8, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 10 : 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {isMobile && (
                    <button onClick={() => setMobileFilterOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '8px 14px', borderRadius: 8, background: 'rgba(212,196,168,0.1)', border: '1px solid rgba(212,196,168,0.2)', color: 'var(--gold)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: isMobile ? 'flex-start' : 'flex-end' }}>
                  {filtersActive && !isMobile && (
                    <button onClick={() => setShowSavePrompt(true)} style={{ background: 'rgba(212,196,168,0.1)', border: '1px solid rgba(212,196,168,0.2)', borderRadius: 8, padding: '6px 12px', color: 'var(--gold)', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <Bookmark size={13} /> Save
                    </button>
                  )}
                  {savedSearches.length > 0 && !isMobile && (
                    <button onClick={() => setShowSavedPanel(p => !p)} style={{ background: showSavedPanel ? 'rgba(212,196,168,0.15)' : 'rgba(255,255,255,0.04)', border: `1px solid ${showSavedPanel ? 'rgba(212,196,168,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 8, padding: '6px 12px', color: showSavedPanel ? 'var(--gold)' : 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <Search size={13} /> Saved ({savedSearches.length})
                    </button>
                  )}
                  {filters.brand && (
                    <button onClick={() => onBrandChange('All')} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9999, padding: '5px 10px', color: 'rgba(255,255,255,0.45)', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>
                      ✕ {filters.brand}
                    </button>
                  )}
                  <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: '5px 8px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.6)', fontSize: 10, fontWeight: 600, cursor: 'pointer', outline: 'none', maxWidth: isMobile ? 120 : 'auto' }}>
                  ...
                  <div style={{ display: 'flex', gap: 3, background: 'rgba(255,255,255,0.04)', padding: 3, borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
                    <button onClick={() => setViewMode('grid')} style={{ padding: '5px 9px', borderRadius: 6, border: 'none', cursor: 'pointer', background: viewMode === 'grid' ? 'rgba(212,196,168,0.15)' : 'transparent', color: viewMode === 'grid' ? 'var(--gold)' : 'rgba(255,255,255,0.3)', transition: 'all 0.2s', display: 'flex', alignItems: 'center' }}>
                      <LayoutGrid size={15} />
                    </button>
                    <button onClick={() => setViewMode('list')} style={{ padding: '5px 9px', borderRadius: 6, border: 'none', cursor: 'pointer', background: viewMode === 'list' ? 'rgba(212,196,168,0.15)' : 'transparent', color: viewMode === 'list' ? 'var(--gold)' : 'rgba(255,255,255,0.3)', transition: 'all 0.2s', display: 'flex', alignItems: 'center' }}>
                      <List size={15} />
                    </button>
                  </div>
                </div>
              ...
              {showSavePrompt && (
                <div style={{ background: '#0C0C0C', border: '1px solid rgba(212,196,168,0.2)', borderRadius: 12, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <Bookmark size={16} style={{ color: 'var(--gold)', flexShrink: 0 }} />
                  <input value={saveName} onChange={e => setSaveName(e.target.value)} placeholder="Name this search..." style={{ flex: 1, minWidth: 160, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '7px 10px', color: '#fff', fontSize: 12, outline: 'none' }} onKeyDown={e => e.key === 'Enter' && handleSaveSearch()} autoFocus />
                  <button onClick={handleSaveSearch} className="btn btn-gold btn-sm">Save</button>
                  <button onClick={() => { setShowSavePrompt(false); setSaveName(''); }} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', fontSize: 11 }}>Cancel</button>
                </div>
              )}

              {showSavedPanel && savedSearches.length > 0 && (
                <div style={{ background: '#0C0C0C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, marginBottom: 16, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Saved Searches</div>
                  {savedSearches.map(s => (
                    <div key={s._id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <button onClick={() => handleApplySearch(s)} style={{ flex: 1, textAlign: 'left', background: 'none', border: 'none', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>{s.name}</button>
                      <button onClick={() => handleToggleNotify(s._id, s.notify)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: s.notify ? 'var(--gold)' : 'rgba(255,255,255,0.2)', display: 'flex', padding: 4 }} title={s.notify ? 'Notifications on' : 'Notifications off'}>
                        {s.notify ? <Bell size={13} /> : <BellOff size={13} />}
                      </button>
                      <button onClick={() => handleDeleteSearch(s._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.2)', display: 'flex', padding: 4 }} title="Delete"><Trash2 size={13} /></button>
                    </div>
                  ))}
                </div>
              )}
