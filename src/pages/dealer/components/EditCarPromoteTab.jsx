...
export default function EditCarPromoteTab({ car, images, coverImage, handlePromote }) {
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(15, 23, 42, 0.07)', borderRadius: 16, padding: '24px' }}>
      <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'rgba(15, 23, 42, 0.3)', marginBottom: 18 }}>Homepage Promotion</div>
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(15, 23, 42, 0.4)', lineHeight: 1.65, marginBottom: 20 }}>
            {car.isPromoted
          ...
          <button onClick={handlePromote} style={{ padding: '11px 24px', background: car.isPromoted ? 'rgba(239,68,68,0.1)' : 'rgba(37, 99, 235,0.12)', border: car.isPromoted ? '1px solid rgba(239,68,68,0.2)' : '1px solid rgba(37, 99, 235,0.25)', borderRadius: 10, color: car.isPromoted ? '#ef4444' : 'var(--gold)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
            <Star size={14} fill={car.isPromoted ? '#ef4444' : 'none'} />
            {car.isPromoted ? 'Remove from Homepage' : 'Feature on Homepage'}
          </button>
        </div>
        <div style={{ width: 180, height: 120, borderRadius: 12, overflow: 'hidden', background: '#FFFFFF', flexShrink: 0 }}>
          {images[coverImage] && (
