  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(15, 23, 42, 0.07)', borderRadius: 16, padding: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'rgba(15, 23, 42, 0.3)', marginBottom: 4 }}>Photo Management</div>
          <p style={{ fontSize: 12, color: 'rgba(15, 23, 42, 0.3)', margin: 0 }}>Tap to select, click pin to set cover, or delete selected.</p>
        </div>
      ...
      {images.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', border: '2px dashed rgba(15, 23, 42, 0.08)', borderRadius: 12, marginTop: 16 }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>no image</div>
          <div style={{ color: 'rgba(15, 23, 42, 0.35)', fontSize: 14, marginBottom: 16 }}>No photos yet. Click "Upload Photos" to add images.</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 165px), 1fr))', gap: 12, marginTop: 16 }}>
          {images.map((img, i) => {
            ...
            return (
              <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: isCover ? '2px solid var(--gold)' : isSelected ? '2px solid #3b82f6' : '2px solid rgba(15, 23, 42, 0.07)', aspectRatio: '4/3', background: '#FFFFFF', cursor: 'pointer', transition: 'border-color 0.2s', opacity: isDeleting ? 0.5 : 1 }}>
                ...
                <div style={{ position: 'absolute', inset: 0, background: isCover ? 'rgba(37, 99, 235,0.1)' : isSelected ? 'rgba(59,130,246,0.1)' : 'transparent', transition: 'background 0.2s' }} onClick={() => toggleSelect(i)} role="presentation" />
                <div aria-label={isCover ? "Cover image" : "Set as cover"} style={{ position: 'absolute', top: 6, left: 6, width: 28, height: 28, borderRadius: 7, background: isCover ? 'var(--gold)' : 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                  ...
                  <Pin size={13} style={{ color: isCover ? '#000' : 'rgba(15, 23, 42, 0.6)' }} />
                </div>
                <div aria-label="Delete image" style={{ position: 'absolute', top: 6, right: 6, width: 28, height: 28, borderRadius: 7, background: 'rgba(239,68,68,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                  ...
                  role="button" tabIndex={0}>
                  <Trash2 size={13} style={{ color: '#0F172A' }} />
                </div>
                <div style={{ position: 'absolute', bottom: 6, right: 6, width: 28, height: 28, borderRadius: 7, background: isSelected ? '#3b82f6' : 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: isSelected ? 'none' : '1px solid rgba(15, 23, 42, 0.2)' }}
                  ...
                  role="button" tabIndex={0} aria-label={isSelected ? `Deselect image ${i + 1}` : `Select image ${i + 1}`}>
                  {isSelected ? <span style={{ color: '#0F172A', fontSize: 14, fontWeight: 900 }}>x</span> : <span style={{ color: 'rgba(15, 23, 42, 0.5)', fontSize: 10 }}>{i + 1}</span>}
                </div>
                {isCover && (
