...
import { ShieldCheck, Search, QrCode, FileText, CheckCircle, Clock, Lock, Smartphone, Users, Star, ChevronRight } from 'lucide-react';
...
const FAQ = [
  ...
  { q: 'What if the inspection finds issues?', a: 'You\'ll get a full breakdown of any issues found, complete with severity ratings. Use this information to negotiate with the seller or walk away from a bad deal.' },
  { q: 'Is there a money-back guarantee?', a: 'If the inspector is unable to access the vehicle within 7 days of payment (through no fault of yours), we issue a full refund.' },
];
...
export default function GhostCheckerInfo() {
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      {/* ── HERO ── */}
      <div style={{
        background: 'linear-gradient(180deg, rgba(212,196,168,0.06) 0%, transparent 100%)',
        borderBottom: '1px solid var(--border)', padding: '72px 32px 64px',
      }}>
        <div style={{ maxWidth: 780, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ width: 64, height: 64, borderRadius: 16, background: 'rgba(212,196,168,0.1)', border: '1px solid rgba(212,196,168,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/showroom" style={{ padding: '12px 28px', background: 'var(--gold)', color: '#000', borderRadius: 9999, fontWeight: 800, fontSize: 12, letterSpacing: '0.04em', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
              Browse Vehicles <ChevronRight size={14} />
            </Link>
            <a href="#how-it-works" style={{ padding: '12px 28px', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)', borderRadius: 9999, fontWeight: 700, fontSize: 12, textDecoration: 'none' }}>
              See How It Works
            </a>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 32px' }}>
            ...
            {STEPS.map((s, i) => (
              ...
              >
                <div style={{
                  ...
                  background: 'rgba(212,196,168,0.1)', border: '1px solid rgba(212,196,168,0.12)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                }}>
                  <s.icon size={20} style={{ color: 'var(--gold)' }} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ fontSize: 14, fontWeight: 700, color: '#fff', margin: '0 0 4px' }}>{s.title}</h3>
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6, margin: 0 }}>{s.desc}</p>
                </div>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(212,196,168,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 11, fontWeight: 900, color: 'var(--gold)', fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>
                  {i + 1}
                </div>
              </div>
            ))}
        ...
        <div style={{ padding: '0 0 64px' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <span style={{ fontSize: 9, color: 'var(--gold)', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Why Use It</span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 3vw, 2rem)', color: '#fff', margin: '8px 0' }}>Peace of Mind, Every Purchase</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 220px), 1fr))', gap: 14 }}>
            {FEATURES.map((f, i) => (
              <div key={i} style={cardStyle}>
                <div style={{ padding: '24px 22px' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(212,196,168,0.08)', border: '1px solid rgba(212,196,168,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
        ...
        <div style={{ padding: '0 0 64px' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <span style={{ fontSize: 9, color: 'var(--gold)', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Pricing</span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 3vw, 2rem)', color: '#fff', margin: '8px 0' }}>Choose Your Inspection</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: 16 }}>
            {PRICING.map((p, i) => (
              <div key={i} style={{
                ...cardStyle,
                borderColor: p.popular ? 'rgba(212,196,168,0.3)' : 'var(--border)',
                position: 'relative',
              }}>
                {p.popular && (
                  <div style={{
                    position: 'absolute', top: 12, right: 12,
                    padding: '3px 10px', borderRadius: 9999, background: 'rgba(212,196,168,0.12)',
                    border: '1px solid rgba(212,196,168,0.2)', fontSize: 8, fontWeight: 800,
                    color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.08em',
                  }}>Most Popular</div>
                )}
                <div style={{ padding: '28px 24px' }}>
                  <h3 style={{ fontSize: 15, fontWeight: 700, color: '#fff', margin: '0 0 4px' }}>{p.name}</h3>
                  <div style={{ fontSize: '1.8rem', fontFamily: 'var(--font-display)', fontWeight: 900, fontStyle: 'italic', color: 'var(--gold)', margin: '12px 0' }}>{p.price}</div>
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', lineHeight: 1.7, margin: '0 0 20px' }}>{p.desc}</p>
                  <Link to={`/showroom`} style={{
                    display: 'block', textAlign: 'center', padding: '10px 0',
                    borderRadius: 8, fontWeight: 800, fontSize: 11,
                    background: p.popular ? 'var(--gold)' : 'rgba(255,255,255,0.04)',
                    border: p.popular ? 'none' : '1px solid rgba(255,255,255,0.1)',
                    color: p.popular ? '#000' : 'rgba(255,255,255,0.5)',
                    textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.06em',
                    transition: 'all 0.2s',
                  }}
                    onMouseEnter={e => { if (!p.popular) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.2)'; e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; } }}
                    onMouseLeave={e => { if (!p.popular) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; } }}>
                    Get Started
                  </Link>
                </div>
              </div>
        ...
        <div style={{ padding: '0 0 72px' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <span style={{ fontSize: 9, color: 'var(--gold)', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' }}>FAQ</span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 3vw, 2rem)', color: '#fff', margin: '8px 0' }}>Common Questions</h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {FAQ.map((f, i) => (
              <details key={i} style={{
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; }}
              >
                <summary style={{ fontSize: 13, fontWeight: 700, color: '#fff', listStyle: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  {f.q}
                  <ChevronRight size={14} style={{ color: 'rgba(255,255,255,0.25)', transition: 'transform 0.2s' }} />
                </summary>
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, margin: '12px 0 0', maxWidth: 600 }}>{f.a}</p>
              </details>
            ))}
          </div>
        </div>
