      ...
      try {
        const observer = new PerformanceObserver((list) => {
          list.getEntries().forEach((entry) => {
            if (import.meta.env.DEV) console.log('[Performance]', entry.name, entry.duration);
          });
        });
...
export function useRenderTime(componentName: string): () => void {
  if (typeof performance !== 'undefined') {
    ...
    return () => {
      ...
      if (import.meta.env.DEV) console.log(`[Render Time] ${componentName}: ${duration.toFixed(2)}ms`);
    };
  }
  return () => {};
}
