        ...
        <div className="overflow-x-auto border border-slate-200 rounded-xl">
          <Table>
            ...
            <TableBody>
              ...
              <TableRow className={highlightDifferences && areValuesDifferent((v) => v.escrowEligible) ? 'bg-amber-50/70' : ''}>
                <TableCell className="font-bold text-slate-600 bg-slate-50/50">Escrow Vault</TableCell>
                {vehicles.map((v) => (
                  <TableCell key={v.id}>
                    <Badge variant={v.escrowEligible ? 'escrow' : 'neutral'}>
                      {v.escrowEligible ? '✓ Escrow Vault Ready' : 'Standard'}
                    </Badge>
                  </TableCell>
                ))}
              </TableRow>
