...
const checks = [
  ['ledger has provider payable account', fs.readFileSync(path.join(root,'supabase/migrations/20260921134954_v14_holistic_financial_corrections.sql'),'utf8').includes("'5100'")],
  ...
  ['settlement periods are idempotent', fs.readFileSync(path.join(root,'supabase/migrations/20260918150000_inspection_financial_workflow_completion.sql'),'utf8').includes('period_start=p_period_start') && fs.readFileSync(path.join(root,'supabase/migrations/20260921134954_v14_holistic_financial_corrections.sql'),'utf8').includes('Settlement already paid with a different payment reference')],
  ['payout posts to canonical ledger', fs.readFileSync(path.join(root,'supabase/migrations/20260921134954_v14_holistic_financial_corrections.sql'),'utf8').includes("'inspection_payout'") && fs.readFileSync(path.join(root,'supabase/migrations/20260921134954_v14_holistic_financial_corrections.sql'),'utf8').includes("'5100','1000'")],
