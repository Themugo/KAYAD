...
check('Private seller stale fabricated consumers removed', !/\boffers\.filter\b|\binspectionRequests\.length\b|\bcompletedSales\.length\b/.test(read('src/features/PrivateSellerPlatform/pages/PrivateSellerPlatform.tsx')));
...
check('Canonical payment history has no synthetic wallet balance variables', !/\{balance\}|\{pendingBalance\}/.test(read('src/features/PaymentHistoryView.tsx')));
