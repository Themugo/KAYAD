  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const elite = all.filter(c => c.auctionStatus === 'live' || c.allowBid).slice(0, 3);
        const rest = all.filter(c => !(c.auctionStatus === 'live' || c.allowBid)).slice(0, 6);
        setFeatured(elite);
        setRecent(rest);
        setStats({
  ...
  return (
    <div className="page" style={{ background: '#050505' }}>
      ...
      <section style={{ borderTop: '1px solid rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: 'rgba(255,255,255,0.04)' }}>
          {[
            { label: 'Vehicles Listed', value: stats ? `${stats.totalCars}+` : '...' },
            { label: 'Live Auctions', value: stats ? `${stats.liveAuctions}` : '...' },
            { label: 'Brands Available', value: stats ? `${stats.brands}` : '...' },
            { label: 'Avg. Price', value: stats ? `KES ${(stats.avgPrice / 1000000).toFixed(1)}M` : '...' },
          ].map((s, i) => (
      ...
      {loading && (
        <section style={{ padding: '60px 0' }}>
          <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 28 }}>
            {[1, 2, 3].map(i => (
