...
export default function EditCarPage() {
  ...
  useEffect(() => {
    carsAPI.get(id).then(d => {
      ...
      setForm({
        ...
        color: c.color || '', city: c.location?.city || c.city || '',
        description: c.description || '', dealerPhone: c.dealerPhone || '',
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const { city: _city, ...rest } = form;
      let images = car?.images?.filter((_, i) => !removedImages.includes(i)) || [];
      if (newImages.length > 0) {
        const newUrls = newImages.map(f => URL.createObjectURL(f));
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <Field label="Body Type">
                  <input className="input" value={form.bodyType} onChange={e => set('bodyType', e.target.value)} />
                </Field>
                <Field label="Color">
          ...
          <div className="card" style={{ padding: 28 }}>
            ...
            {car.images?.length > 0 && (
              <div className="grid-4" style={{ gap: 8, marginBottom: 16 }}>
                {car.images.map((img, i) => {
                  const deleted = removedImages.includes(i);
                  return (
                    <div key={i} style={{ aspectRatio: '4/3', borderRadius: 8, overflow: 'hidden', position: 'relative', background: 'var(--surface)', border: `2px solid ${deleted ? '#ff4444' : i === coverImage ? 'var(--gold)' : 'transparent'}`, opacity: deleted ? 0.35 : 1 }}>
                      <img src={img.url || img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', cursor: 'pointer' }} onClick={() => !deleted && setCoverImage(i)} />
                      {i === coverImage && !deleted && (
                      ...
                      <button onClick={() => setRemovedImages(p => deleted ? p.filter(x => x !== i) : [...p, i])}
                        style={{ position: 'absolute', top: 4, right: 4, width: 24, height: 24, borderRadius: '50%', border: 'none', cursor: 'pointer', fontSize: 12, background: deleted ? '#444' : 'rgba(255,68,68,0.85)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {deleted ? '↺' : '✕'}
                      </button>
                    </div>
                  );
            ...
            {previews.length > 0 && (
              <div className="grid-4" style={{ gap: 8, marginTop: 12 }}>
                {previews.map((src, i) => (
                  <div key={i} onClick={() => setCoverImage((car.images?.length || 0) - removedImages.length + i)} style={{ aspectRatio: '4/3', borderRadius: 8, overflow: 'hidden', background: 'var(--surface)', cursor: 'pointer', border: `2px solid ${(car.images?.length || 0) - removedImages.length + i === coverImage ? 'var(--gold)' : 'transparent'}`, transition: 'all 0.15s' }}>
            ...
            {removedImages.length > 0 && (
              <div style={{ marginTop: 8, fontSize: 11, color: '#ff4444' }}>{removedImages.length} image(s) marked for deletion</div>
            )}
          </div>
