...
export default function Navbar() {
  const { user, isAuth, isAdmin, isDealer, isSeller, isBroker, isSuperAdmin, logout } = useAuth();
      ...
      <nav style={{
        ...
        <div className="container" style={{ display: 'flex', alignItems: 'center', gap: 20, width: '100%' }}>
          ...
          <div className="desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: 18, flex: 1 }}>
            ...
            {isBroker && <NavLink to="/dealer">My Listings</NavLink>}
                  ...
                  {userDrop && (
                    <div style={{
                      ...
                      {[
                        ...
                        ...(!isDealer && !isBroker && !isAdmin ? [{ to: '/dashboard', icon: '🏠', label: 'My Dashboard' }] : []),
                        ...
                        ...(isBroker ? [{ to: '/dealer', icon: '📋', label: 'My Listings' }] : []),
                        ...(isAdmin  ? [{ to: '/admin', icon: '🔑', label: 'Admin Panel' }] : []),
                        ...(isSuperAdmin || user?.role === 'accounts' ? [{ to: '/admin/transactions', icon: '💳', label: 'Transactions' }] : []),
                        ...(isSuperAdmin ? [{ to: '/admin/panic-room', icon: '🚨', label: 'Panic Room' }] : []),
                        ...(isSuperAdmin || user?.role === 'marketing' || user?.role === 'ad_manager' ? [{ to: '/admin/ads', icon: '📢', label: 'Ad Manager' }] : []),
                      ].map(item => (
          ...
          <div className="mobile-menu-panel" onClick={e => e.stopPropagation()}>
            ...
            {isAuth && (
              <>
                ...
                {!isDealer && !isBroker && !isAdmin && <NavLink to="/dashboard" mobile>🏠 My Dashboard</NavLink>}
            ...
            {isSeller && (
              <>
                <div style={{ padding: '8px 20px 4px', fontSize: 10, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 8 }}>
                  {isDealer ? 'Dealer' : 'Seller'}
                </div>
                <NavLink to="/dealer"           mobile>{isDealer ? '🏪 Dealer Hub' : '📋 My Listings'}</NavLink>
                <NavLink to="/dealer/add-car"   mobile>➕ Add Listing</NavLink>
                {isDealer && <NavLink to="/dealer/analytics" mobile>📊 Analytics</NavLink>}
              </>
            )}
            ...
            {isAdmin && (
              <>
                <div style={{ padding: '8px 20px 4px', fontSize: 10, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 8 }}>
                  {user?.role === 'superadmin' ? 'Super Admin' : user?.role === 'marketing' ? 'Marketing' : user?.role === 'technical_support' ? 'Support' : user?.role === 'hr' ? 'HR' : user?.role === 'accounts' ? 'Accounts' : user?.role === 'escrow_officer' ? 'Escrow' : user?.role === 'ad_manager' ? 'Ad Manager' : user?.role === 'moderator' ? 'Moderator' : 'Admin'}
                </div>
                <NavLink to="/admin" mobile>📊 Dashboard</NavLink>
                {(isSuperAdmin || user?.role === 'hr' || user?.role === 'technical_support') && <NavLink to="/admin/users" mobile>👥 Users</NavLink>}
                {(isSuperAdmin || user?.role === 'technical_support' || user?.role === 'moderator') && <NavLink to="/admin/cars" mobile>🚗 Cars</NavLink>}
                {(isSuperAdmin || user?.role === 'hr') && <NavLink to="/admin/sellers" mobile>🏪 Dealer Approvals</NavLink>}
                {(isSuperAdmin || user?.role === 'accounts' || user?.role === 'escrow_officer') && <NavLink to="/admin/transactions" mobile>💳 Transactions</NavLink>}
                {(isSuperAdmin || user?.role === 'accounts' || user?.role === 'escrow_officer') && <NavLink to="/admin/escrows" mobile>🔒 Escrow Ledger</NavLink>}
                {(isSuperAdmin || user?.role === 'marketing' || user?.role === 'ad_manager') && <NavLink to="/admin/ads" mobile>📢 Ad Manager</NavLink>}
                {isSuperAdmin && <NavLink to="/admin/panic-room" mobile>🚨 Panic Room</NavLink>}
                {isSuperAdmin && <NavLink to="/admin/settings" mobile>⚙ Settings</NavLink>}
              </>
            )}
